...
namespace UnrealBuildTool
{
	...
	class ApplePostBuildSyncMode : ToolMode
	{
		...
		private int PostBuildSync(ApplePostBuildSyncTarget Target, ILogger Logger)
		{
			...
			// Read the same automatic-signing setting that BuildWithStubXcodeProject
			// and the modern Xcode project generator use.
			ConfigHierarchy SharedPlatformIni = ConfigCache.ReadHierarchy(
			...
			SharedPlatformIni.TryGetValue(
				"/Script/MacTargetPlatform.XcodeProjectSettings",
				"bUseAutomaticCodeSigning",
				out bUseAutomaticCodeSigning);

			// Legacy stub signing creates a temporary keychain and forces:
			// CODE_SIGN_STYLE=Manual
			// CODE_SIGN_IDENTITY=...
			// PROVISIONING_PROFILE_SPECIFIER=...
			//
			// Do not enter that path when Xcode automatic signing is enabled.
			bool bUseLegacyStubSigning =
				bCreateStub &&
				!bUseDummySigning &&
				!bUseAutomaticCodeSigning;

			// Remote Windows builds still need the uploaded .p12 imported into an
			// unlocked keychain, even when Xcode itself performs automatic signing.
			bool bUseRemoteKeychainSetup =
				bCreateStub &&
				!bUseDummySigning;
			
			Logger.LogInformation(
				"ApplePostBuildSync signing mode: Automatic={Automatic}, LegacyStub={LegacyStub}, Dummy={Dummy}",
				bUseAutomaticCodeSigning,
				bUseLegacyStubSigning,
				bUseDummySigning);

			if (bUseRemoteKeychainSetup)
			{
				ExtraOptions += SetupRemoteCodesigning(
					Target,
					bUseAutomaticCodeSigning);
			}
		...
		private static string SetupRemoteCodesigning(
    ...
    bool bUseAutomaticCodeSigning)
		{
			...
			using (StreamWriter Writer = new StreamWriter(SignProjectScript.FullName))
			{
				...
				Writer.WriteLine(
					"security list-keychains -d user -s \"{0}\" \"$HOME/Library/Keychains/login.keychain-db\"",
					TempKeychain);
			...
			{
				// The certificate/private key has been installed into an unlocked
				// temporary keychain, but Xcode remains responsible for choosing the
				// identity and managed provisioning profile.
				return "";
			}
