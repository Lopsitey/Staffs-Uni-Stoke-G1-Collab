...
Console.WriteLine("target receipt executable path: {0}", InExecutablePath);
	...
	public override void Package(ProjectParams Params, DeploymentContext SC, int WorkingCL)
	{
		...
		if (UnrealBuildTool.BuildHostPlatform.Current.Platform != UnrealTargetPlatform.Mac)
		{
			...
			if (!Params.IterativeDeploy || !File.Exists(ProjectIPA) || bNeedsIPA)
			{
				...
				// IPhonePackager rejects Xcode-managed profiles during its normal profile
				// search. The staging process also recreates the staged directory, so copy
				// the profile after staging and immediately before RepackageFromStage runs.
				Logger.LogInformation(
					"iOS provision staging hook reached: NeedToSign={NeedToSign}, AutomaticSigning={AutomaticSigning}",
					bNeedToSign,
					bAutomaticSigning);

				if (TargetPlatformType == UnrealTargetPlatform.IOS &&
				    bNeedToSign)
				{
					FileReference ProjectProvision = FileReference.Combine(
						Params.RawProjectPath.Directory,
						"Build",
						"IOS",
						Params.ShortProjectName + ".mobileprovision");

					FileReference EmbeddedProvision = FileReference.Combine(
						SC.StageDirectory,
						"embedded.mobileprovision");

					if (!FileReference.Exists(ProjectProvision))
					{
						throw new AutomationException(
							"Automatic iOS signing profile was not found at {0}",
							ProjectProvision.FullName);
					}

					DirectoryReference.CreateDirectory(EmbeddedProvision.Directory);
					FileReference.Copy(ProjectProvision, EmbeddedProvision, true);

					Logger.LogInformation(
						"Copied automatic iOS provision into staged package: {Provision}",
						EmbeddedProvision.FullName);
				}
	...
	private void CodeSign(string BaseDirectory, string GameName, FileReference RawProjectPath, UnrealTargetConfiguration TargetConfig, string LocalRoot, string ProjectName, string ProjectDirectory, bool IsCode, bool Distribution, string Provision, string Certificate, string Team, bool bAutomaticSigning, string SchemeName, string SchemeConfiguration)
	{
		...
		else
		{
			...
			if (!string.IsNullOrEmpty(Provision))
			{
				...
				{
					string UUID = "";
					string AllText = File.ReadAllText(MobileProvisionDir.FullName);
	...
	public override void Deploy(ProjectParams Params, DeploymentContext SC)
	{
		...
		if (Params.DeviceNames[0].Contains(AppleToolChainSettings.LocalMacUUID) || Params.bMakeMacNative)
		{
			...
			String Arg1 = FileReference.FromString(AppToDeploy).Directory.FullName;														// Path to staged ios folder
			String Arg2 = FileReference.Combine(FileReference.FromString(AppToDeploy).Directory.ParentDirectory, "iOSonMac").FullName;	// Path to destination folder
			String Arg3 = FileReference.FromString(AppToDeploy).GetFileName();															// App name
	...
	public override IProcessResult RunClient(ERunOptions ClientRunFlags, string ClientApp, string ClientCmdLine, ProjectParams Params)
	{
		if (UnrealBuildTool.BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Mac || UnrealBuildTool.BuildHostPlatform.Current.Platform == UnrealTargetPlatform.Win64)
		{
			...
			if (Params.DeviceNames[0].Contains(AppleToolChainSettings.LocalMacUUID))
			{
				...
				GetDevices().Where(Device => Device.Id == Params.DeviceNames[0] && int.Parse(Device.SoftwareVersion.Split('.').First()) < 17).Count() == 0)
			{
