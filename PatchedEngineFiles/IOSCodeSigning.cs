...
namespace UnrealBuildTool
{
	...
	public class AppleCodeSign
	{
		...
		/// </summary>
		public static readonly string ProvisionDirectory = AppleExports.GetProvisionDirectory().FullName;
		...
		public static List<FileReference> FindProvisions(string CFBundleIdentifier, bool bForDistribution, out FileReference? MatchedProvision)
		{
			...
			{
				_logger?.LogError("Could not find provision directory '{0}'.", ProvisionDirectory);
			...
			foreach (string ProvisionFile in Directory.EnumerateFiles(ProvisionDirectory, "*.mobileprovision"))
			{
	...
	public class MobileProvision
	{
		...
		public static void CacheMobileProvisions()
		{
			...
			string ProvisionDirectory = AppleCodeSign.ProvisionDirectory;
			if (!Directory.Exists(ProvisionDirectory))
			{
				_logger?.LogInformation("Provision Folder {0} doesn't exist, creating..", ProvisionDirectory);
			...
			foreach (string ProvisionCopySrcDirectory in ProvisionCopySrcDirectories)
			{
				if (Directory.Exists(ProvisionCopySrcDirectory))
				{
					...
					{
						string TargetFile = ProvisionDirectory + Path.GetFileName(Provision);
		...
		{
			if (!Directory.Exists(AppleCodeSign.ProvisionDirectory))
			{
				_logger?.LogInformation("Provision Folder {0} doesn't exist, nothing to do.", AppleCodeSign.ProvisionDirectory);
			}
			else
			{
				_logger?.LogInformation("Cleaning out contents of  Provision Folder {0}", AppleCodeSign.ProvisionDirectory);
				foreach (string Provision in Directory.GetFiles(AppleCodeSign.ProvisionDirectory))
				{
		...
		public static string FindCompatibleProvision(string CFBundleIdentifier, out bool bNameMatch, bool bCheckCert = true, bool bCheckIdentifier = true, bool bCheckDistro = true)
		{
			...
			{
				if (File.Exists(ProvisionDirectory + "/" + CodeSigningConfig.Provision))
				{
					return ProvisionDirectory + "/" + CodeSigningConfig.Provision;
				}
			}
			...
			// cache the provision library
			Dictionary<string, MobileProvision> ProvisionLibrary = new Dictionary<string, MobileProvision>();
			foreach (string Provision in Directory.EnumerateFiles(ProvisionDirectory, "*.mobileprovision"))
			{
				MobileProvision? p = MobileProvisionParser.ParseFile(Provision);
				if (p != null)
				{
					ProvisionLibrary.Add(Provision, p);
					if (p.FileName != null && p.UUID != null && p.FileName.Contains(p.UUID) && !File.Exists(Path.Combine(ProvisionDirectory, "UE4_" + p.UUID + ".mobileprovision")))
					{
						File.Copy(Provision, Path.Combine(ProvisionDirectory, "UE4_" + p.UUID + ".mobileprovision"));
						p = MobileProvisionParser.ParseFile(Path.Combine(ProvisionDirectory, "UE4_" + p.UUID + ".mobileprovision"));
						if (p != null)
						{
							ProvisionLibrary.Add(Path.Combine(ProvisionDirectory, "UE4_" + p.UUID + ".mobileprovision"), p);
						}
					}
				}
			}

			_logger?.LogInformation("Searching for mobile provisions that match the game '{0}' (distribution: {3}) with CFBundleIdentifier='{1}' in '{2}'", GameName, CFBundleIdentifier, ProvisionDirectory, CodeSigningConfig.bForDistribution);
			...
			for (int Phase = -1; Phase < 3; ++Phase)
			{
				...
				foreach (string Key in ProfileKeys)
				{
					...
					Path.GetFileName(TestProvision.FileName).ToLower().Equals(TestProvision.UUID.ToLower() + ".mobileprovision"))
					{
						...
						_logger?.LogInformation("  Allowing Xcode-managed profile, baby!!!");
					}
					...
					{
						_logger?.LogInformation("  Allowing automatic provisioning profile, baby!!!");
					}
